    ...
    <div class="page_body">
        ...
        if($_POST['op'] == 'new') {
            if(   $email == $_POST['email'] 
			   && \security\password\strenght($_POST['new_pass']) === true) {
                gdrcd_query("UPDATE personaggio SET pass = '".\security\password\hash($_POST['new_pass'])."', ultimo_cambiopass = NOW() WHERE nome = '".$_SESSION['login']."'");
        ...
        if(gdrcd_filter('get', $_POST['op']) == 'force') {
            if(($_SESSION['permessi'] >= MODERATOR) && (\security\password\strenght($_POST['new_pass']) === true)) {
                if($_SESSION['permessi'] == SUPERUSER) {
                    $query = "UPDATE personaggio SET pass = '".\security\password\hash($_POST['new_pass'])."', ultimo_cambiopass = NOW() WHERE nome = '".gdrcd_filter_in($_POST['account'])."'";
                } else {
                    $query = "UPDATE personaggio SET pass = '".\security\password\hash($_POST['new_pass'])."', ultimo_cambiopass = NOW() WHERE nome = '".gdrcd_filter_in($_POST['account'])."' AND permessi < ".SUPERUSER."";
                }
                gdrcd_query($query);
                /*Registro l'evento */
